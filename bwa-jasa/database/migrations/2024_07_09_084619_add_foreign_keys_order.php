...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('order', function (Blueprint $table) {
            $table->foreign('buyer_id','fk_order_buyer_to_service')->references('id')->on('users')
            ->onUpdate('CASCADE')->onDelete('CASCADE');
            $table->foreign('freelancer_id','fk_order_freelancer_to_service')->references('id')->on('users')
            ->onUpdate('CASCADE')->onDelete('CASCADE');
